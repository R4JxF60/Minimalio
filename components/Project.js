...
function Project(props) {
    ...
    const createDescription = (subheading, description) => {
        return <React.Fragment><h3 className="font-merriweather font-normal md:text-2xl text-xl mb-2">{subheading || "Project Subheading"}</h3><p className="font-lato font-normal text-lg md:text-xl mb-3">{description || "Description"}</p></React.Fragment>;
    } 

    return (
        <div className="md:flex flex-row block mb-16 text-mid-black">
            <div className="w-full h-auto md:w-[454px] md:h-[454px] md:mr-12 overflow-hidden relative">
                <div className={"w-full h-full absolute z-10 flex items-center justify-center "+props.color}>
                    <div className={`font-sen text-${props.color}-500 sm:text-5xl text-3xl bg-light-gray rounded-full h-fit w-fit sm:p-8 p-5`}>{props.logo || "logo"}</div>
                </div>
                <Image 
                    src={props.src} 
                    width={454}
            ...
            <div className="md:w-[55%] w-full">
                ...
                {createDescription(props.sub, props.description)}
